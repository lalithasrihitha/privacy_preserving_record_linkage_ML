...
plt.figure()
plt.plot(fpr, tpr, label='SVM ROC (AUC = %0.4f)' % roc_auc)
plt.plot([0, 1], [0, 1], 'k--')
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('ROC Curve -SVM upsample')
plt.legend(loc="lower right")
plt.savefig("SVM_upsample_roc.png")
plt.show()
...
plt.title('Precision-Recall Curve - SVM upsample')
plt.savefig("SVM_upsample_pr.png")
plt.show()
...
pd.DataFrame(y_pred).to_csv("svm_upsample_pred.csv", index=False)
