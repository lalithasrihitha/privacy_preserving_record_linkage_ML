...
upsampled = upsampled.sample(frac=1, random_state=42).reset_index(drop=True)
  GNU nano 2.9.8                                               pprl_knn_upsample.py
...
plt.figure()
plt.plot(fpr, tpr, label='KNN ROC (AUC = %0.4f)' % roc_auc)
plt.plot([0, 1], [0, 1], 'k--')
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('ROC Curve - KNN upsample')
plt.legend(loc="lower right")
plt.savefig("Knn_upsample_roc.png")
plt.show()
...
plt.title('Precision-Recall Curve - KNN upsample')
plt.savefig("Knn_upsample_pr.png")
plt.show()
...
pd.DataFrame(y_pred).to_csv("knn_upsample_pred.csv", index=False)
