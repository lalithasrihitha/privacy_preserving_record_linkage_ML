...
X = pd.read_csvv("data/sample_data/binary_features.csv")
...
plt.figure()
plt.plot(fpr, tpr, label='SNN ROC (AUC = %0.4f)' % roc_auc)
plt.plot([0, 1], [0, 1], 'k--')
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('ROC Curve - SNN')
plt.legend(loc="lower right")
plt.savefig("snn_roc.png")
plt.show()
...
plt.title('Precision-Recall Curve - SNN')
plt.savefig("slnn_pr.png")
plt.show()
...
pd.DataFrame(y_pred).to_csv("slnn_original_pred.csv", index=False)
